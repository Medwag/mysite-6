...
$w.onReady(async () => {
  ...
  try {
    ...
    // 3) Prefill inputs
    $w('#inDrName').value       = originalItem.homeGp || '';
    $w('#inDrNumber').value       = originalItem.gpNumber || '';
    $w('#inDrEmergencyNumber').value     = originalItem.doctorsEmergencyNumber || '';
  ...
  $w('#btnSave').onClick(async () => {
    ...
    const name = ($w('#inDrName').value || '').trim();
    const number = ($w('#inDrNumber').value || '').trim();
    ...
    try {
      ...
      const updates = {
        _id: recordId,
        homeGp: name,
        gpNumber: ($w('#inDrNumber').value || '').trim() || null,
        doctorsEmergencyNumber: number,
